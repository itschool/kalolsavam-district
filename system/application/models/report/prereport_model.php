...
/*	 function list_of_participants_in_group($reg_no,$item_code)
	{ 
	    $this->db->select('pid.parent_admn_no');
		$this->db->from('participant_item_details AS pid');
		$this->db->where('pid.item_code',$item_code);
		$this->db->where('pid.participant_id',$reg_no);
		$school_details		=	$this->db->get();
		return $school_details->result_array();
	}*/
	function list_of_participants_in_group_final($reg_no,$item_code)
	{
					$sql="SELECT `pid`.`participant_id` , `pd`.`participant_name` , `pd`.`class` , `pd`.`gender` , `m`.`school_code` , `m`.`school_name` , `f`.`fest_id` , `f`.`fest_name` , `pid`.`parent_admn_no` , `im`.`item_code` , `im`.`item_name`
			FROM (
			`participant_item_details` AS pid
			)
			JOIN `participant_details` AS pd ON `pid`.`participant_id` = `pd`.`participant_id`
			...
			AND `pid`.`parent_admn_no`
			IN (
			
			SELECT parent_admn_no
			FROM participant_item_details
			WHERE participant_id = $reg_no
			)";
	...
	function team_list_in_group_final($cap,$item_code)
	{
	
	   if($cap != 'ALL')
	   {
			$sql="SELECT `pid`.`participant_id` , `pd`.`participant_name` , `pd`.`class` , `pd`.`gender`,pid.is_captain,pid.parent_admn_no,`m`.`school_code` , `m`.`school_name` , `f`.`fest_id` , `f`.`fest_name` , `pid`.`parent_admn_no` , `im`.`item_code` , `im`.`item_name`
			...
			WHERE `pid`.`item_code` = $item_code
			AND `pid`.`parent_admn_no`
			IN (SELECT parent_admn_no FROM participant_item_details
			WHERE admn_no = $cap) order by `m`.`school_code`, `pid`.parent_admn_no ASC,pid.is_captain DESC,`pid`.`participant_id`  ASC";
		}
		else
		{
			$sql="SELECT `pid`.`participant_id` , `pd`.`participant_name` , `pd`.`class` , `pd`.`gender` ,pid.is_captain,pid.parent_admn_no, `m`.`school_code` , `m`.`school_name` , `f`.`fest_id` , `f`.`fest_name` , `pid`.`parent_admn_no` , `im`.`item_code` , `im`.`item_name`
			...
			JOIN `festival_master` AS f ON `f`.`fest_id` = `im`.`fest_id`
			JOIN `school_master` AS m ON `m`.`school_code` = `pid`.`school_code`
			WHERE `pid`.`item_code` = $item_code order by `m`.`school_code`, `pid`.parent_admn_no ASC,pid.is_captain DESC,`pid`.`participant_id`  ASC";		
		}
		   $fest_detail1		=	$this->db->query($sql);
		    return $fest_detail1->result_array();
	}
...
//return $school_details->result_array();
	...
	$this->db->select(' SM.stage_name, SM.stage_desc, IM.item_code, IM.item_name, FM.fest_id, FM.fest_name, CP.participant_id, CP.cluster_no, SIM.start_time, SIM.item_time, SIM.time_type,SIM.no_of_participant,pd.spo_id,so.is_publish');
		...
		$this->db->join('participant_item_details AS pd','pd.participant_id=CP.participant_id AND pd.item_code = CP.item_code');
...
pd.class, pd.gender, pid.item_code, pid.item_type, pid.is_captain, it.item_name, it.fest_id, sm.stage_id, date(sm.start_time) AS datee, ssm.stage_desc,ssm.stage_name, fm.fest_name,cpd.cluster_no');
		...
		$this->db->join('cluster_participant_details AS cpd','cpd.item_code = pid.item_code AND cpd.participant_id = pid.participant_id','LEFT'); 
	...
	$this->db->from('school_details AS sd');
		$this->db->select('sd.school_code,mj.school_name, pd.participant_id, pd.school_code, pd.participant_name,
pd.class, pd.gender, pid.item_code, pid.item_type, pid.is_captain, it.item_name, it.fest_id, sm.stage_id,sm.start_time as timer,sm.start_time AS datee, ssm.stage_desc,ssm.stage_name, fm.fest_name,cpd.cluster_no, pd.sub_district_code,SDM.sub_district_name,pd.admn_no');
		$this->db->join('school_master AS mj','mj.school_code = sd.school_code');
		$this->db->join('participant_details AS pd','pd.school_code = sd.school_code');
		$this->db->join('participant_item_details  AS pid ','pid.participant_id = pd.participant_id');
		$this->db->join('item_master AS it','it.item_code = pid.item_code');
	...
	function cluster_reportreport($festid,$itemcode)
	{
		...
		$this->db->select('cm.stage_id, cm.item_code, cm.cluster_id, cm.cluster_no, cpd.participant_id, cpd.cluster_no, fm.fest_name, im.item_name, sm.stage_name, sm.stage_desc, (sim.start_time) AS stime,im.max_time,time( sim.start_time) AS ttime');
		$this->db->from('cluster_master AS cm');
		$this->db->join('stage_item_master AS sim','cm.item_code = sim.item_code');
	...
	function cluster_report_date($festid,$date)
	{
		$this->db->select('cm.stage_id, cm.item_code, cm.cluster_id, cm.cluster_no, cpd.participant_id, cpd.cluster_no, fm.fest_name,fm.fest_id, im.item_name, sm.stage_name, sm.stage_desc, (sim.start_time) AS stime,im.max_time,time( sim.start_time ) AS ttime,date(sim.start_time)');
		$this->db->from('cluster_master AS cm');
		$this->db->join('stage_item_master AS sim','cm.item_code = sim.item_code');
		$this->db->join('cluster_participant_details AS cpd','cm.item_code = cpd.item_code');
		$this->db->join('item_master  AS  im','im.item_code = cpd.item_code');
	...
	function cluster_report_date_all($date)
	{
		$this->db->select('cm.stage_id, cm.item_code, cm.cluster_id, cm.cluster_no, cpd.participant_id, cpd.cluster_no, fm.fest_name,fm.fest_id, im.item_name, sm.stage_name, sm.stage_desc, (sim.start_time) AS stime,im.max_time,time( sim.start_time ) AS ttime,date(sim.start_time)');
		$this->db->from('cluster_master AS cm');
		$this->db->join('stage_item_master AS sim','cm.item_code = sim.item_code');
	...
	{
		$this->db->select('cm.stage_id,sim.stage_id, cm.item_code, cm.cluster_id, cm.cluster_no, cpd.participant_id, cpd.cluster_no, fm.fest_name,fm.fest_id, im.item_name, sm.stage_name, sm.stage_desc, (sim.start_time) AS stime,im.max_time,time( sim.start_time ) AS ttime,date(sim.start_time)');
		$this->db->from('cluster_master AS cm');
		$this->db->join('stage_item_master AS sim','cm.item_code = sim.item_code');
		$this->db->join('cluster_participant_details AS cpd','cm.item_code = cpd.item_code');
		$this->db->join('item_master  AS  im','im.item_code = cpd.item_code');
		...
		$this->db->group_by('cpd.participant_id');
		$this->db->group_by('cm.item_code');
		$this->db->order_by('cpd.item_code');
		$this->db->order_by('cpd.cluster_no');
		//$this->db->order_by('cpd.item_code');
			$fest_details		=	$this->db->get();
			return $fest_details->result_array();
	...
	function cluster_report_stage($festid,$stage)
	{
		$this->db->select('sim.stage_id,cm.item_code, cm.cluster_id, cm.cluster_no, cpd.participant_id, cpd.cluster_no, fm.fest_name,fm.fest_id, im.item_name, sm.stage_name, sm.stage_desc, (sim.start_time) AS stime,im.max_time,time( sim.start_time ) AS ttime,date(sim.start_time)');
		$this->db->from('cluster_master AS cm');
		$this->db->join('stage_item_master AS sim','cm.item_code = sim.item_code');
		$this->db->join('cluster_participant_details AS cpd','cm.item_code = cpd.item_code');
		$this->db->join('item_master  AS  im','im.item_code = cpd.item_code');
	...
	function cluster_report_stage_all($stage)
	{
		$this->db->select('sim.stage_id,cm.item_code, cm.cluster_id, cm.cluster_no, cpd.participant_id, cpd.cluster_no, fm.fest_name,fm.fest_id, im.item_name, sm.stage_name, sm.stage_desc, (sim.start_time) AS stime,im.max_time,time( sim.start_time ) AS ttime,date(sim.start_time)');
		$this->db->from('cluster_master AS cm');
		$this->db->join('stage_item_master AS sim','cm.item_code = sim.item_code');
	...
	{
		$this->db->where('im.fest_id',$festid);
		$this->db->select('min(cm.start_time) as cm_start_time, cm.stage_id, cm.item_code, cm.cluster_id, cm.cluster_no, cpd.participant_id, cpd.cluster_no as clustno, fm.fest_name,fm.fest_id, im.item_name, sm.stage_name, sm.stage_desc, (sim.start_time) AS stime,im.max_time,time( sim.start_time ) AS ttime');
		$this->db->from('cluster_master AS cm');
		$this->db->join('stage_item_master AS sim','cm.item_code = sim.item_code');
		$this->db->join('cluster_participant_details AS cpd','cm.item_code = cpd.item_code');
		$this->db->join('item_master  AS  im','im.item_code = cpd.item_code');
...
ORDER BY PD.participant_id,CM.start_time,CM.end_time";
...
$fest_detail		=	$this->db->get();
   ...
   $fest_detail1		=	$this->db->query($que);
	...
	function timesheet($itemcode)
	{
		...
		$this->db->select('IT.*,SM.stage_name,SM.stage_desc,date(ST.start_time) as ttime');
		$item_details		=	$this->db->get();
		return $item_details->result_array();
	}
	 ...
	 */
	  ...
	  function  fetch_fest_scoresheet($fest_id,$date)
		...
		{
		$this->db->select('p.participant_name, p.class, p.school_code, p.gender, p.participant_id, pd.item_code, pd.item_type, pd.is_captain, m.school_name,m.school_code,sdm.sub_district_code,sdm.sub_district_name,rdm.rev_district_name,rdm.rev_district_code,I.item_name, f.fest_name');
		$this->db->from('participant_details AS p');
		...
		$this->db->join('sub_district_master AS sdm','m.sub_district_code = sdm.sub_district_code');
		 $this->db->join('rev_district_master AS rdm','m.rev_district_code = sdm.rev_district_code');
		...
		function datewise_allfestival($festid)
		{
 ...
 function list_all_school_details()
  ...
  function higherlevel_result($festid,$item)
	...
	function list_participants_datewise($fest,$date)
	{
     $this->db->select('p.participant_id,d.admn_no,p.school_code, p.item_code, p.item_type, p.is_captain, d.participant_name, d.gender, d.class, date( s.start_time ),s.start_time, im.time_type, im.item_name, F.fest_id,F.fest_name, m.school_name,su.sub_district_name,rd.rev_district_name, s.no_of_participant, im.is_off_stage,im.max_time,p.spo_id,so.is_publish');
		$this->db->from('participant_item_details AS p');
		$this->db->join('item_master AS im','im.item_code = p.item_code');
		$this->db->join('festival_master AS F','F.fest_id = im.fest_id');
		$this->db->join('school_master AS m',' m.school_code = p.school_code');
		$this->db->join('sub_district_master AS su',' m.sub_district_code = su.sub_district_code');
		$this->db->join('rev_district_master AS rd',' m.rev_district_code = rd.rev_district_code');
		$this->db->join('participant_details AS d','d.participant_id = p.participant_id');
		//$this->db->join('cluster_participant_details AS c','p.participant_id = c.participant_id and c.item_code = p.item_code','LEFT');
		$this->db->join('stage_item_master AS s','s.item_code = p.item_code');
		$this->db->join('special_order_master AS so','so.spo_id=p.spo_id','LEFT');
		//$this->db->join('stage_master AS ss','ss.stage_id = s.stage_id','LEFT');
		//$this->db->where('p.school_code',$schoolcode);
		//$this->db->where('m.rev_district_code',$rev_district_code);
		$this->db->where('p.is_captain','Y');
		if($fest!=0){
		$this->db->where('F.fest_id',$fest);
		}
		if($date!='All')
		{
        $this->db->where('date(s.start_time)',$date);
		}
		$this->db->group_by('p.participant_id');
		$this->db->group_by('p.item_code');
		$this->db->order_by('p.item_code');
	...
	function list_participants_stagewise($date,$stage)
	{
     $this->db->select('p.participant_id,d.admn_no,p.school_code, p.item_code, p.item_type, p.is_captain, d.participant_name, d.gender, d.class, date( s.start_time ),s.start_time, ss.stage_name,ss.stage_id,ss.stage_desc,im.time_type, im.item_name, F.fest_id,F.fest_name, m.school_name,su.sub_district_name,rd.rev_district_name, s.no_of_participant, im.is_off_stage,im.max_time,p.spo_id,so.is_publish');
		$this->db->from('participant_item_details AS p');
		$this->db->join('item_master AS im','im.item_code = p.item_code');
		$this->db->join('festival_master AS F','F.fest_id = im.fest_id');
		$this->db->join('school_master AS m',' m.school_code = p.school_code');
		...
		//$this->db->join('cluster_participant_details AS c','p.participant_id = c.participant_id and c.item_code = p.item_code','LEFT');
		$this->db->join('stage_item_master AS s','s.item_code = p.item_code');
		$this->db->join('stage_master AS ss','ss.stage_id = s.stage_id');
		$this->db->join('special_order_master AS so','so.spo_id=p.spo_id','LEFT');
		//$this->db->where('p.school_code',$schoolcode);
		//$this->db->where('m.rev_district_code',$rev_district_code);
		$this->db->where('p.is_captain','Y');
		...
		if($stage!='All')
		{
        $this->db->where('ss.stage_id',$stage);
		}
		$this->db->group_by('p.participant_id');
		$this->db->group_by('p.item_code');
		$this->db->order_by('p.item_code');
	...
	function call_date_stage($date,$stage)
	{
		$this->db->select('SM.stage_name,SM.stage_id, SM.stage_desc, IM.item_code, IM.item_name, FM.fest_id, FM.fest_name, CP.participant_id, CP.cluster_no, SIM.start_time, SIM.item_time, SIM.time_type,SIM.no_of_participant,pd.spo_id,so.is_publish');
		$this->db->where('SIM.item_code = CP.item_code');
		$this->db->from('stage_item_master  AS SIM');
		$this->db->join('stage_master AS SM ','SM.stage_id = SIM.stage_id');
		$this->db->join('item_master    AS IM ','IM.item_code = SIM.item_code');
		...
		if($stage!='All')
		{
		$this->db->where('SM.stage_id',$stage);
		}
		//$this->db->where('CP.item_code',$itemcode);
		$this->db->group_by('CP.participant_id');
		$this->db->group_by('IM.item_code');
		$this->db->order_by('IM.item_code');
		$this->db->order_by('CP.participant_id');
		$school_details		=	$this->db->get();
		return $school_details->result_array();
	}
	function time_date_stage($date,$stage)
	{
	...
	function score_date_stage($date,$stage)
	{
	...
	function tabulation_date_stage($date,$stage)
	{
